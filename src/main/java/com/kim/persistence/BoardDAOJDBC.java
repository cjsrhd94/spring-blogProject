...
public class BoardDAOJDBC implements BoardDAO {
    ...
    private final String BOARD_INFO = "select p.post_id, p.title, p.content, p.created_date, p.category_id, c.display_type " +
                                             "from post as p join category as c " +
                                             "on c.category_id = p.category_id " +
                                             "where c.blog_id like ? order by p.post_id desc";
    private final String BOARD_INFO_SELECT = "select p.post_id, p.title, p.content, p.created_date, p.category_id, c.display_type " +
                                             "from post as p join category as c " +
                                             "on c.category_id = p.category_id " +
    ...
    public void insertBoard(BoardVO vo) {
        System.out.println("===> SPRING JDBC 기반으로 insertBoard() 기능 처리");
    ...
    public void updateBoard(BoardVO vo) {
        System.out.println("===> SPRING JDBC 기반으로 updateBoard() 기능 처리");
    ...
    public void deleteBoard(BoardVO vo) {
        System.out.println("===> SPRING JDBC 기반으로 deleteBoard() 기능 처리");
    ...
    public BoardVO getBoard(BoardVO vo) {
        System.out.println("===> SPRING JDBC 기반으로 getBoard() 기능 처리");
    ...
    public List<Map> getBoardInfo(CategoryVO categoryVO) {
        System.out.println("===> SPRING JDBC 기반으로 getBoardInfo() 기능 처리");
        if (categoryVO.getCategoryId() == null) {
            Object[] params = {categoryVO.getBlogId()};
            return spring.query(BOARD_INFO, params, new BoardInfoRowMapper());
        } else {
            Object[] params = {categoryVO.getBlogId(), categoryVO.getCategoryId()};
            return spring.query(BOARD_INFO_SELECT, params, new BoardInfoRowMapper());
        }
    }
}
